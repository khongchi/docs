...
Laravel allows you to easily define a single route to handle every action in a controller using simple, REST naming conventions. 라라벨에서는 REST풀한 라우팅 룰을 처리하는 컨트롤러 액션을 정의하기 위한 손쉬운 방법을 제공합니다. First, define the route using the `Route::controller` method: 먼저 `Route::controller` 메소드를 사용하여 경로를 지정합니다.
...
A catch-all method may be defined which will be called when no other matching method is found on a given controller. 컨트롤러에서 일치하는 메소드를 찾을 수 없는 경우에 호출되는 메소드가 존재합니다. The method should be named `missingMethod`, and receives the method and parameter array for the request: `missingMethod`라고 불리는 이 메소드는 요청에 대한 파라미터가 있는 경우 이를 배열의 형태로 넘겨 받습니다. 

#### Defining A Catch-All Method 
#### 찾을 수 없는 메소드를 처리하는 missingMethod 
