...
Laravel also includes drivers for the Mailgun and Mandrill HTTP APIs. 라라벨은 또한 Mailgun 과 Mandrill의 HTTP API 드라이버를 제공하고 있습니다. These APIs are often simpler and quicker than the SMTP servers. 이러한 API는 대부분의 경우 SMTP 보다 간단하고 빠릅니다. Both of these drivers require that the Guzzle 4 HTTP library be installed into your application. 두가지의 드라이버를 사용하기 위해서는 Guzzle 4 HTTP 라이브러리를 응용프로그램에 설치해야 합니다. You can add Guzzle 4 to your project by adding the following line to your `composer.json` file: 다음과 같은 라인을 `composer.json` 파일에 추가하여 Guzzle 4 를 프로젝트에 추가할 수 있습니다. 

	"guzzlehttp/guzzle": "~4.0"

#### Mailgun Driver Mailgun 드라이버

To use the Mailgun driver, set the `driver` option to `mailgun` in your `app/config/mail.php` configuration file. Mailgun 드라이버를 사용하려면 `app/config/mail.php` 설정파일에 `driver` 부분을 `mailgun`으로 변경하면 됩니다. Next, create an `app/config/services.php` configuration file if one does not already exist for your project. 다음으로 프로젝트에 `app/config/services.php` 파일이 존재하지 않는다면 새롭게 생성합니다. Verify that it contains the following options: 그 후 다음과 같은 내용을 추가합니다. 
...
#### Mandrill Driver Mandrill 드라이버

To use the Mandrill driver, set the `driver` option to `mandrill` in your `app/config/mail.php` configuration file. Mandrill 드라이버를 사용하려면 `app/config/mail.php` 설정 파일에 `driver` 부분을 `mandrill`으로 변경하면 됩니다. Next, create an `app/config/services.php` configuration file if one does not already exist for your project. 다음으로 프로젝트에 `app/config/services.php` 파일이 존재하지 않는다면 새롭게 생성합니다. Verify that it contains the following options: 그 후 다음과 같은 내용을 추가합니다. 
...
If the `driver` option of your `app/config/mail.php` configuration file is set to `log`, all e-mails will be written to your log files, and will not actually be sent to any of the recipients. `app/config/mail.php` 설정 파일에서 `driver` 옵션을 `log`로 설정한다면 실제로 이메일을 수신자에게 보내지 않고 로그 파일에 기록하게 됩니다. This is primarily useful for quick, local debugging and content verification. 이 설정은 주로 빠르게 로컬에서 디버깅 해야하거나 내용을 확인하고자 할때 유용합니다. 
